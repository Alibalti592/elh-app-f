...
class UserRepository {
  ...
  Future<dynamic> getUserJwtFromRefreshToken(String jwt) async {
    ...
    final data =
        jsonEncode({'refresh_token': json.decode(jwt)['refresh_token']});
  ...
  Future<ApiResponse> resetPassword(email) async {
    Map<String, String> requestHeaders = {'Content-type': 'application/json'};
    return _apiHelper.get('ini-reset-password?email=$email', requestHeaders,
        public: true);
  }
  ...
  Future<ApiResponse> confirmResetPassword(password, code, email) async {
    Map<String, String> requestHeaders = {'Content-type': 'application/json'};
    var body =
        json.encode({"password": password, 'code': code, 'email': email});
    return _apiHelper.post('confirm-reset-password', requestHeaders, body,
        public: true);
  }
}
