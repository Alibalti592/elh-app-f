...
class _JumpingDot extends AnimatedWidget {
  ...
  _JumpingDot({Key? key, required Animation<double> animation, this.color, this.size})
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text('.', style: TextStyle(color: color, fontSize: size),
      ),
    );
  }
}
...
/// default [size] of 10.0, default [color] as black, [dotSpacing] (gap
...
class _JumpingDotsProgressIndicatorState
  ...
  void _addListOfDots(int index) {
    _widgets.add(
      Container(
        margin: EdgeInsets.only(right: dotSpacing!),
        child: _JumpingDot(
